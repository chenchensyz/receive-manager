...
userList.prototype = {
    ...
    initData: function () {
        ...
        that.tableIns = that.layTable.render({
            elem: '#userList'
            , url: getRootPath() + '/companyInfo/getDeveloperListData'
            ...
            , where: {'state': 1, 'companyId': compId}
            , cols: [[
                {type: 'numbers'}
        ...
        that.layTable.on('tool(userTable)', function (obj) {
            ...
            if (obj.event === 'edit') {//编辑
                location.href = getRootPath() + "/companyInfo/getCompanyUser?userId=" + data.id;
    ...
    toAdd: function () {
        $('.add-btn').off('click').on('click', function () {
            ...
            location.href = getRootPath() + '/companyInfo/getCompanyUser?userId=0';
        });

        $('.comp-edit').off('click').on('click', function () {
            location.href = getRootPath() + "/companyInfo/getCompanyInfo?compId=-1";
        });
    }
};
new userList();