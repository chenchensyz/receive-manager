        ...
        if (appServiceId > 0) {
            $.get(getRootPath() + '/appService/getAppServiceData?appServiceId=' + appServiceId).then(function (res) {
                if (res.code == 0) {
                    ...
                    $('.lastUpdateUser').val(res.data.lastUpdateUser).addClass('input-disabled');
                    $('.lastUpdateTime').val(res.data.lastUpdateTimeStr).addClass('input-disabled');
    ...
    updateForm: function () {
        ...
        this.layForm.on('submit(create)', function (data) {
            $.post(getRootPath() + '/appService/addOrEdit', data.field).then(function (res) {
                if (res.code == 0) {
                    location.href = getRootPath() + '/appService/getAppServiceList?appId=' + appId;
                } else {
