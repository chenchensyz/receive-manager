...
companyList.prototype = {
    ...
    initData: function () {
        ...
        that.tableIns = that.layTable.render({
            elem: '#companyList'
            , url: getRootPath() + '/companyInfo/queryCompanyListData'
            ...
            , done: function (res, curr, count) {
                ...
                $("[data-field='state']").children().each(function () {
                    if ($(this).text() == '1') {
                        $(this).text('可用');
                        $(this).parent().siblings().last().find('.opt-btn').text('禁用').attr('data-type', 0);
                    } else if ($(this).text() == '0') {
                        $(this).text('已禁用');
                        $(this).parent().siblings().last().find('.opt-btn').text('启用').attr('data-type', 1);
                    }
                });
            }
        });
        ...
        //监听工具条
        that.layTable.on('tool(companyTable)', function (obj) {
            var data = obj.data;
            if (obj.event === 'edit') {//编辑
                location.href = getRootPath() + "/companyInfo/getCompanyInfo?compId=" + data.id;
            } else if (obj.event === 'developer') {
                location.href = getRootPath() + "/companyInfo/getDeveloperList?compId=" + data.id;
            } else if (obj.event === 'opt') {
                that.optCompany(data);
            }
        });
